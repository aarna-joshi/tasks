...
export function sumPoints(questions: Question[]): number {
    return questions.reduce(
        (sum: number, q: Question): number => sum + q.points, //not necessary to specify types but helps
        0,
    );
}
...
export function toCSV(questions: Question[]): string {
    const column = "id,name,options,points,published";

    const row = questions.map(
        (q: Question): string =>
...
export function sameType(questions: Question[]): boolean {
    ...
    return questions.every((q: Question): boolean => q.type === first_type);
}
